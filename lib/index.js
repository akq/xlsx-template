...
module.exports = (function() {
    ...
    */
            ...
            substitution.forEach(function(element, idx) {
                ...
                if(idx === 0) { // insert in the row where the placeholders are
                    ...
                    if((idx - 1) < newTableRows.length) {
                        ...
                        var r = self.getCurrentRow(row, newTableRows.length + 1)
                        newRow = self.cloneElement(row, r);
                        // newRow.attrib.r = self.getCurrentRow(row, newTableRows.length + 1);
                        newTableRows.push(newRow);
                    }
                    // var cr = newRow.attrib.r
                    var cr = cell.attrib.r.replace(/\d+/, newRow.attrib.r)

                    newCell = newRow.find("c[@r='"+cr+"']")
                    // Create a new cell
                    ...
                    if(value instanceof Array) {
                        ...
                        newCells.forEach(function(c) {
                            var ec = newRow.find("c[@r='"+c.attrib.r+"']")
                            if(ec)newRow.remove(ec)                                
                            newRow.append(c);
                        });
                        ...
                        var f = newCell.find('f')
                        // var v = newCell.find('v')
                        // if(!v) {
                        //     var nv = etree.Element('v')
                        //     nv.tail = ''
                        //     nv.text = ''
                        //     newCell.append( nv)
                        // }
    ...
    Workbook.prototype.cloneElement = function(element, r) {
        var self = this;

        var newElement = etree.Element(element.tag, element.attrib);
        newElement.text = element.text;
        newElement.tail = element.tail;
        var last = element.attrib.r
        if(typeof r === 'number'){
            if(typeof last === 'number')
                newElement.attrib.r = r
            else if(typeof last === 'string')
                newElement.attrib.r = newElement.attrib.r.replace(/\d+/, r)

        }
        element.getchildren().forEach(function(child) {
            var cloned = self.cloneElement(child, r)
            if(cloned.tag==='v'){
                // cloned.text = null
                return 
            }
            newElement.append(cloned);
        });
// 
        return newElement;
    };
